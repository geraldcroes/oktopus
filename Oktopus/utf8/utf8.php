...
/**
 * A port of Kohana_UTF8 (http://kohanaframework.org/)
 * wich is a port of [phputf8](http://phputf8.sourceforge.net/) to a unified set
...
class UTF8
{
     ...
     * @param   string  character set, defaults to Engine::$charset
     * @return  mixed
    ...
    public static function transliterate_to_ascii($str, $case = 0)
    {
        if (!isset(self::$called[__FUNCTION__])) {
            require SYSPATH . 'utf8' . DIRECTORY_SEPARATOR . __FUNCTION__ . '.php';
    ...
    {
        if (UTF8::$server_utf8)
            return mb_strlen($str, Engine::$charset);
    ...
    {
        if (UTF8::$server_utf8)
            return mb_strpos($str, $search, $offset, Engine::$charset);
    ...
    {
        if (UTF8::$server_utf8)
            return mb_strrpos($str, $search, $offset, Engine::$charset);
    ...
    public static function substr($str, $offset, $length = NULL)
    {
        if (UTF8::$server_utf8)
            return ($length === NULL)
                    ? mb_substr($str, $offset, mb_strlen($str), Engine::$charset)
                    : mb_substr($str, $offset, $length, Engine::$charset);
    ...
    {
        if (UTF8::$server_utf8)
            return mb_strtolower($str, Engine::$charset);
    ...
    {
        if (UTF8::$server_utf8)
            return mb_strtoupper($str, Engine::$charset);
    ...
    {
        if (!isset(self::$called[__FUNCTION__])) {
            require OKTOPUS_PATH . 'utf8' . DIRECTORY_SEPARATOR . __FUNCTION__ . '.php';
